$(document).ready(function() {
  ...
  initialize();
		...
		switch(buttonID) {
		 ...
		 case 'book-appt-ready':
			...
			case 'new-construction-yes':
				...
				showHide(['#cup-sup-ext'],[]);
				break;

			case 'new-construction-no':

				showHide(['#interior-only'],[]);
				break;

			case 'interior-only-yes':

			  showHide(['#cup-sup-int'],[])
				break;

			case 'interior-only-no':
			...
			case 'cup-sup-int-yes':

				showHide(['#finished-use-only'], [])
				break;
	...
	function codeAddress(address) {
	   ...
	   $('#loader').html("<h3>Please wait</h3><p>We're checking to see if your address is in the City of San Jose.</p>");
	...
	$('#print').click(function() {
		...
		$('#test-pdf').prepend("<h4>Here's what you need to include in your Planning Application.</h4>")
